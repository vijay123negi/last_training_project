...
const ItemCard: React.FC<ItemCardProps> = ({ params }) => {
  ...
  const handleFormSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const updatedItem = {
      ...
      storeServiceIds: JSON.parse(
        formData.get("storeServiceIds") as string
      ) as number[],
      yearOfService: Number(formData.get("yearOfService")),
      timezone: formData.get("timezone") as string,
      knowledgeOnBrands: JSON.parse(
        formData.get("knowledgeOnBrands") as string
      ) as number[],
        ...
        <CardContent>
          <Grid container spacing={2}>
            <Grid item xs={12} md={10}>
              <Typography variant="h5">Item ID: {id}</Typography>
              <Typography variant="body1">Name: {name}</Typography>
              <Typography variant="body1">Email: {email}</Typography>
              <Typography variant="body1">PhoneNo: {phoneNo}</Typography>
